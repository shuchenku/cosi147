package pbservice

import "net"
import "fmt"
import "net/rpc"
import "log"
import "time"
import "viewservice"
import "sync"
import "os"
import "syscall"
import "math/rand"


type PBServer struct {
  mu sync.Mutex
  l net.Listener
  dead bool // for testing
  unreliable bool // for testing
  me string
  vs *viewservice.Clerk
  // Your declarations here.
  viewnum uint
  isPrimary bool
  isBackup bool
  backup string   // backup's port
  kv map[string]string
}

func (pb *PBServer) Get(args *GetArgs, reply *GetReply) error {

  pb.mu.Lock()
  defer pb.mu.Unlock()
  // Your code here.
  if (!pb.isPrimary && !pb.isBackup) || (pb.isBackup && !args.FromPrimary) || (pb.isPrimary && args.FromPrimary) {
    reply.Err = ErrWrongServer
  } else {
    var ok bool
    if (pb.isPrimary) {
      args.FromPrimary = true
      ok = call(pb.backup, "PBServer.Get", args, &reply) 
    }

    if (ok && reply.Err==OK) || (pb.isPrimary && pb.backup == "") || pb.isBackup {
      value, hasKey := pb.kv[args.Key]
      if !hasKey {
        reply.Err = ErrNoKey
      } else {
        reply.Value = value
        reply.Err = OK      
      }
    } 
  }
  return nil
}

func (pb *PBServer) Put(args *PutArgs, reply *PutReply) error {
  // Your code here.
  pb.mu.Lock()
  defer pb.mu.Unlock()

  if (!pb.isPrimary && !pb.isBackup) || (pb.isBackup && !args.FromPrimary) || (pb.isPrimary && args.FromPrimary) {
    reply.Err = ErrWrongServer
  } else {
    var ok bool
    if (pb.isPrimary) {
      args.FromPrimary = true
      ok = call(pb.backup, "PBServer.Put", args, &reply) 
    }
    if (ok && reply.Err==OK) || (pb.isPrimary && pb.backup == "") || pb.isBackup  {
      pb.kv[args.Key] = args.Value
      reply.Err = OK
    }
  }
  return nil
}

//
// ping the viewserver periodically.
// if view changed:
//   transition to new view.
//   manage transfer of state from primary to new backup.
//
func (pb *PBServer) tick() {
  // Your code here.
  pb.mu.Lock()
  defer pb.mu.Unlock()

  var pingViewnum uint
  if pb.isPrimary || pb.isBackup {
    pingViewnum = pb.viewnum
  } else {
    pingViewnum = 0
  }
  view, err := pb.vs.Ping(pingViewnum)

  if err != nil { 
    return 
  }

  if (view.Viewnum != pb.viewnum) {
    pb.viewnum = view.Viewnum
    if (view.Primary == pb.me) {
      pb.backup = view.Backup
      if pb.isPrimary {
        //transfer state
        args := TransferArgs{}
        reply := TransferReply{}
        args.State = pb.kv
        for !(call(view.Backup,"PBServer.Transfer", args, &reply) && reply.Err==OK) {
        }
      } else {
        pb.isPrimary = true
        pb.isBackup = false
      }
    } else if (view.Backup == pb.me) {
      pb.isPrimary = false
      pb.isBackup = true
      pb.backup = view.Backup
    } else {
      pb.isPrimary = false
      pb.isBackup = false
    }
  }
}


func (pb *PBServer) Transfer(args *TransferArgs, reply *TransferReply) error {
  if !pb.isBackup {
    reply.Err = ErrWrongServer
  } else {
    pb.kv = args.State
    reply.Err = OK
  }
  return nil
}



// tell the server to shut itself down.
// please do not change this function.
func (pb *PBServer) kill() {
  pb.dead = true
  pb.l.Close()
}


func StartServer(vshost string, me string) *PBServer {
  pb := new(PBServer)
  pb.me = me
  pb.vs = viewservice.MakeClerk(me, vshost)
  // Your pb.* initializations here.
  pb.kv = make(map[string]string)

  rpcs := rpc.NewServer()
  rpcs.Register(pb)

  os.Remove(pb.me)
  l, e := net.Listen("unix", pb.me);
  if e != nil {
    log.Fatal("listen error: ", e);
  }
  pb.l = l

  // please do not change any of the following code,
  // or do anything to subvert it.

  go func() {
    for pb.dead == false {
      conn, err := pb.l.Accept()
      if err == nil && pb.dead == false {
        if pb.unreliable && (rand.Int63() % 1000) < 100 {
          // discard the request.
          conn.Close()
        } else if pb.unreliable && (rand.Int63() % 1000) < 200 {
          // process the request but force discard of reply.
          c1 := conn.(*net.UnixConn)
          f, _ := c1.File()
          err := syscall.Shutdown(int(f.Fd()), syscall.SHUT_WR)
          if err != nil {
            fmt.Printf("shutdown: %v\n", err)
          }
          go rpcs.ServeConn(conn)
        } else {
          go rpcs.ServeConn(conn)
        }
      } else if err == nil {
        conn.Close()
      }
      if err != nil && pb.dead == false {
        fmt.Printf("PBServer(%v) accept: %v\n", me, err.Error())
        pb.kill()
      }
    }
  }()

  go func() {
    for pb.dead == false {
      pb.tick()
      time.Sleep(viewservice.PingInterval)
    }
  }()

  return pb
}
